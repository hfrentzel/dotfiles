...
local git_state = function()
    ...
    if vim.fn.filereadable(git_path .. 'MERGE_HEAD') ~= 0 then
        ...
        vim.g.merging_branch = vim.fn.matchstr(
...
local next_conflict_marker = function()
    vim.fn.search(conflict_regex)
end
local prev_conflict_marker = function()
    vim.fn.search(conflict_regex, 'b')
end
